...
function FloatingTabBar(props: TabListProps) {
  ...
  const idleOpacity = morph.interpolate({
    inputRange: [0, 0.42, 1],
    outputRange: [1, 0, 0],
  });
  const idleTranslate = morph.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -14],
  });
  const activeOpacity = morph.interpolate({
    inputRange: [0, 0.58, 1],
    outputRange: [0, 0, 1],
  });
  const activeTranslate = morph.interpolate({
    inputRange: [0, 1],
    outputRange: [14, 0],
  });
...
function GlassBackdrop() {
  ...
  return <View style={[StyleSheet.absoluteFill, styles.pillFallback]} />;
}
...
function RecordingControls() {
  ...
  const primaryIcon = isSent
    ? { android: 'mic' as AndroidSymbol, ios: 'mic.fill' as SFSymbol, web: 'mic' as AndroidSymbol }
    : isRecording
...
const styles = StyleSheet.create({
  ...
  pillFallback: {
    backgroundColor: 'rgba(8, 12, 23, 0.84)',
  ...
  timer: {
    color: TEXT,
    fontFamily: Platform.select({ ios: 'Menlo', android: 'monospace', web: 'ui-monospace' }),
    fontSize: 12,
    fontWeight: '800',
    minWidth: 36,
    textAlign: 'center',
